...
def state_wise_news_for_each_department(state,department,language,max_results,past_no_of_hours):
    ...
    if len(json_resp) != 0:
        ...
        df.columns = ['title', 'description', 'published_date', 'url', 'publisher']
        df['published_date']  = [ datetime.strptime(i, date_format) for i in df['published_date']]
        df["published_time_ago"] = [ time_age_function(i)  for i in df['published_date']]
...
states_list = ['Delhi', ]
# 'Mumbai', 'Hyderabad', 'Chennai', 'Chandigarh', 'Kolkata', 'Bengaluru', 'Bhubaneswar', 'Ahmedabad', 'Guwahati', 'Thiruvananthpuram', 'Imphal', 'Aizawl', 'Agartala', 'Gangtok', 'Kohima', 'Shillong', 'Itanagar', 'Lucknow', 'Bhopal', 'Jaipur', 'Patna', 'Ranchi', 'Shimla', 'Raipur']
text_content = ["President's Secretariat", "Vice President's Secretariat", "Prime Minister's Office", "Cabinet", "Cabinet Committee Decisions", "Cabinet Committee on Economic Affairs (CCEA)", ]
...
def collect_data_for_state(data):
    global text_content,states_list
...
def news_scarpe_from_gnews_final():
    global states_list,text_content
